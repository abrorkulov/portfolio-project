...
export const techMeta = {
  ...
  // simple-icons has no React Native mark; it reuses React's, which is
  // correct — the platform badge is the same atom.
  'React Native': { color: '#61DAFB', slug: 'react', mono: 'RN' },
  GSAP: { color: '#0AE448', slug: 'greensock', mono: 'GS' },
  ...
  'Express.js': { color: '#C7D0DB', slug: 'express', mono: 'Ex' },
  ...
  Prisma: { color: '#2D3748', slug: 'prisma', mono: 'Pr' },

  // ── AI & Systems ──────────────────────────────────────────
  'Claude Code': { color: '#D97757', slug: 'claude', mono: 'Cl' },
  Gemini: { color: '#8E7CF3', slug: 'googlegemini', mono: 'Gm' },
...
/**
 * Every technology the site is allowed to name.
 *
 * The data layer types its skill names, project stacks and journey badges as
 * `TechName`, so a typo — or a technology added to `content` but never given a
 * brand entry here — is a compile error instead of a silent grey `?` tile
 * discovered by a visitor. Adding a technology means adding it here first,
 * which is also where the reminder to verify its CDN slug lives.
 */
export type TechName = keyof typeof techMeta

const FALLBACK: TechMeta = { color: '#7C8B9C', mono: '?', icon: Code2 }

/**
 * Deliberately accepts any string rather than only `TechName`.
 *
 * The data layer is type-checked, but this is also reached from user-supplied
 * strings (the code playground prints them, and `TechIcon` is a general-purpose
 * component), and a runtime lookup that can throw is worse than one that draws
 * a neutral tile.
 */
export function getTechMeta(name: string): TechMeta {
  return (techMeta as Record<string, TechMeta>)[name] ?? FALLBACK
}
