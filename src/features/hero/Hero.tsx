...
import Hero3DFallback from '@/features/hero/Hero3DFallback'
import { useCanSupport3D } from '@/features/hero/useCanSupport3D'
import { useMotionProfile } from '@/shared/motion/useMotionProfile'
import { useMagnetic } from '@/shared/motion/pointerFx'
import { playTick } from '@/shared/lib/fx'
import { profile } from '@/data/content'
import { ArrowDown, Sparkles, Zap } from 'lucide-react'
import { ease, isLiteMotion, lineReveal, staggerParent } from '@/shared/motion/motion'
...
const Hero3D = lazy(() => import('@/features/hero/Hero3D'))
...
/**
 * The headline, one masked line per entry.
 *
 * Two lines rather than the three it used to run. "Frontend / & AI/Systems /
 * Developer" put a two-word line between two one-word lines, which at
 * `clamp(2.5rem, 8vw, 4.5rem)` left a ragged left-aligned block with a hole in
 * the middle of it. Splitting the same title in half gives two lines of
 * comparable measure at every width between 320px and 1920px.
 */
const headline = [
  { text: 'Full-Stack', className: 'text-sheen' },
  {
    text: '& AI Engineer',
    className:
...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* The two big blurred discs that used to sit here are gone.

          They were compensating for a background that turned out to be
          invisible — `body::before` was being painted over by an opaque
          wrapper. With the page wash actually showing, a `blur-3xl` circle on
          top of it stops reading as ambience and starts reading as a grey
          balloon, because a 96px blur radius on a 384px circle still has a
          discernible edge. The wash does this job across the whole page now. */}
